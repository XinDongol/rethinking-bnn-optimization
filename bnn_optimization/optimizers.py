...
class XinBop(tf.keras.optimizers.Optimizer):

    def __init__(self, fp_optimizer, threshold=1e-7, gamma=1e-2, name="XinBop", **kwargs):
    ...
    def apply_gradients(self, grads_and_vars, name=None):
        ...
        print("BINARY VARS: ", [var.name for _, var in bin_grads_and_vars])
        print("FP VARS: ", [var.name for _, var in fp_grads_and_vars])
        ...
        return tf.group(bin_train_op, fp_train_op, name="train_with_xinbop")
    ...
    def _resource_apply_dense(self, grad, var):
        ...
        ifchangesign, new_m_notchangesign, new_m_changesign = self.updata_m(gamma, grad, m, var, threshold)
        
        m_t = tf.compat.v1.assign(m, new_m_notchangesign, use_locking=self._use_locking)
        var_t = lq.math.sign(-tf.sign(var * (m_t+new_m_changesign) - threshold) * var)
        
        return tf.compat.v1.assign(var, var_t, use_locking=self._use_locking).op
    
    def updata_m(self, gamma, grad, m, var, threshold):
        m_update = (1 - gamma) * m + gamma * grad
        ifchangesign = lq.math.sign(var * m_update - threshold) # 1 for change, -1 for not change
        return ifchangesign, m_update*((ifchangesign-1)*-0.5), m_update*((ifchangesign+1)*0.5)
